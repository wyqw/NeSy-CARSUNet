...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        per_class_iu = np.diag(hist) / np.maximum((hist.sum(1) + hist.sum(0) - np.diag(hist)), 1)
        self.iu = iu
        self.miou = iu.mean().item() * 100
        return acc_global, acc, iu, per_class_iu
    ...
    def __str__(self):
        acc_global, acc, iu, per_class_iu = self.compute()
        return (
            'global correct: {:.5f}\n'
            'average row correct: {}\n'
            'IoU: {}\n'
            'mean IoU: {:.5f}').format(
                acc_global.item() * 100,
                ['{:.5f}'.format(i) for i in (acc * 100).tolist()],
                ['{:.5f}'.format(i) for i in (iu * 100).tolist()],
                iu.mean().item() * 100)
